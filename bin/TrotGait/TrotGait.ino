...
RobotDog dog;
TrotGaitCoordinator gait_coordinator(&dog);
...
#define COMMAND_CHECK_TIMER_PERIOD 2
...
void loop() {
    balancer.setDesiredOrientation(gait_coordinator.getCurrentRotation());
    balancer.operate();
    doPIDBalancing();
...
void processSerialInput() {
  if (buttons_read.indexOf("RZ1") > 0) {
    ...
    if (buttons_read.indexOf("B2") > 0) {
        ...
        gait_coordinator.toggleFollowExternalOrientation();
        gait_coordinator.toggleMotionInGroundFrame();
    }
}
...
void doPIDBalancing() {
    ...
    if (do_balancing) {
        ...
        // balancer.setDesiredOrientation(gait_coordinator.getCurrentOrientation());
        Rot balancing_orientation = balancer.getNextKinematicBalancingOrientation();
        gait_coordinator.modifyBodyOrientation(balancing_orientation);
    }
}

void decideAndSendNextCommandToCoordinator() {
    if (!do_balancing && !do_reorientation){
         gait_coordinator.modifyBodyOrientation(ROT_ZERO);
    }

    if (gait_coordinator.isWaiting()) {
        if (do_reorientation) {
            if (orient_command_timer.timeOut()) {
                gait_coordinator.modifyBodyOrientation(command_orientation);
                orient_command_timer.reset();
            }
        }
        else if (command_recieved == MOTION) {
            gait_coordinator.sendMotionCommand(desired_motion.x_motion, 
            ...
            if (leg_return_iterator < 4) {
                gait_coordinator.sendMotionCommandUsingFoot(leg_return_iterator, 0, 0, 0);
                leg_return_iterator++;
            } else if (!has_returned) {
                gait_coordinator.sendReturnToCOMCommand();  
                has_returned = true;
                leg_return_command_started = false;
                leg_return_iterator = 0;
            }
